        ...
        <div className="flex flex-col gap-1">
          {categories.map((category) => (
            <div key={category} className="flex justify-between w-56">
              ...
              <span className="text-cyan-400 font-semibold">${pivotData[category] ? Object.values(pivotData[category]).reduce((sum, v) => sum + (v || 0), 0).toLocaleString(undefined, { maximumFractionDigits: 0 }) : '0'}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          ...
          {categories.length === 0 && (
            <tr>
              <td colSpan={monthsOrder.length + 1} className="text-center py-4 text-gray-400">
